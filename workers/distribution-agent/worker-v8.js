...
function mastodonBaseUrl(env) {
  ...
  if (parsed.pathname !== "/" && parsed.pathname !== "") {
    throw new Error("MASTODON_BASE_URL must be an instance origin without a path");
  }
...
async function mastodonJson(env, path, { method = "GET", body, idempotencyKey } = {}) {
  ...
  if (!response.ok || payload?.error) {
    throw providerError("mastodon", response, payload, `Mastodon API request failed: ${method} ${path}`);
  }
  return payload;
}
...
async function publishMastodon(env, content) {
  const created = await mastodonJson(env, "/api/v1/statuses", {
    ...
    body: {
      status: content.text,
      ...(content.mastodon_visibility ? { visibility: content.mastodon_visibility } : {}),
    },
  ...
  return {
    ok: true,
    target: "mastodon",
    mode: "prepared-post-statuses-plus-requery-v8",
    id: String(created.id),
    permalink: verified.url || created.url || null,
    verification: {
      id: String(verified.id || created.id),
      url: verified.url || created.url || null,
      created_at: verified.created_at || created.created_at || null,
      visibility: verified.visibility || created.visibility || null,
    },
  };
}
...
async function handleCommonPublishWithMastodon(request, env, ctx) {
  ...
  const normalizedTargets = Array.isArray(body?.targets)
    ? [...new Set(body.targets.map(value => String(value).toLowerCase()))]
    : [];
  ...
  if (visibility && !new Set(["public", "unlisted", "private", "direct"]).has(visibility)) {
    return json({ ok: false, error: "mastodon_visibility must be public, unlisted, private, or direct" }, 400);
  }
  ...
  if (body?.dry_run === true) {
    ...
    if (otherTargets.length) {
      const response = await baseWorker.fetch(
        forwardedJsonRequest(request, { ...body, targets: otherTargets, dry_run: true }),
        env,
        ctx
      );
    ...
    return json({
      ...
      plan: {
        ...basePlan,
        mastodon: "OAuth user token with write:statuses -> POST /api/v1/statuses -> GET /api/v1/statuses/:id",
      },
      content: {
        text,
  ...
  let mastodonResult;
  try {
    mastodonResult = await publishMastodon(env, {
      text,
      mastodon_visibility: visibility,
      idempotency_key: body?.idempotency_key ? String(body.idempotency_key) : null,
    });
  } catch (error) {
    mastodonResult = { ok: false, target: "mastodon", error: publicError(error) };
  }
  ...
  if (otherTargets.length) {
    const response = await baseWorker.fetch(
      forwardedJsonRequest(request, { ...body, targets: otherTargets }),
      env,
      ctx
    );
    baseStatus = response.status;
    basePayload = await response.json().catch(() => ({
      ok: false,
      results: {},
      error: { message: `Base Distribution Agent returned HTTP ${response.status}` },
    }));
  }
  ...
  async fetch(request, env, ctx) {
    ...
    if (path === "/health") {
      ...
      return json({
        ...
        mastodon_adapter: "prepared-unverified",
        mastodon_base_url_configured: Boolean(env.MASTODON_BASE_URL),
        mastodon_access_token_configured: Boolean(env.MASTODON_ACCESS_TOKEN),
      });
    }
    ...
    if (path === "/v1/publish" && request.method === "POST") {
      return handleCommonPublishWithMastodon(request, env, ctx);
    }
